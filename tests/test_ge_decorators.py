...
def test_render_ge_tex_decorators_apply():
    ...
    tex = render_ge_tex(matrices=matrices, decorators=decorators, formatter=str)
...
def test_render_ge_tex_decorators_strict_raises_on_empty():
    matrices = [[None, [[1, 2], [3, 4]]]]
    decorators = [{"grid": (0, 1), "entries": [(9, 9)], "decorator": make_decorator(boxed=True)}]

    try:
        render_ge_tex(matrices=matrices, decorators=decorators, formatter=str, strict=True)
    except ValueError:
        return
...
def test_render_ge_tex_respects_explicit_matrices_over_spec():
    from matrixlayout.ge import render_ge_tex

    spec = {"matrices": [[None, [[9]]]]}
    tex = render_ge_tex(matrices=[[None, [[1]]]], spec=spec, formatter=lambda x: f"v{x}")
    assert "v1" in tex
